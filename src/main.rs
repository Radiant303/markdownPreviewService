...
impl SvgBuilder {
    ...
    fn add_list_item(&mut self, text: &str, prefix: &str) {
        let max_chars = self
            .max_chars_proportional(BODY_FONT_SIZE)
            .saturating_sub(4);
    ...
    fn add_blockquote(&mut self, text: &str) {
        ...
        let raw_lines = wrap_text(text, max_chars);
    ...
    fn render_text_run(
        ...
        visible_units(text) * font_size * 0.55
    }
        ...
        while i < chars.len() {
            if chars[i] == start_marker {
                if let Some(end) = chars[i + 1..].iter().position(|&ch| ch == end_marker) {
                    ...
                    let marker_units = hex_decode_string(&hex)
                        .map(|latex| latex.chars().map(visual_units).sum::<f32>() * 0.9)
                        .unwrap_or(4.0);

                    if units + marker_units > max_units && has_visible_text(&line) {
